...
$(function () {
  ...
  $("#og-in-stoic-wallet").click(async function () {
  ...
  async function create_icat() {
    ...
    catInfo = await icat.createCatInfo(principal_, timestamp, gender);
    if (catInfo == null) {
      alert("icat create fail!");
      showLoading(false);
    } else {
      alert("icat create success!");
      var nft = getArrayFromSrc("demo-cat.svg", catInfo, createCatNft);
  ...
  function getArrayFromSrc(src, catInfo, resolve) {
  ...
  async function createCatNft(catInfo, array) {
    var catNft = await icat.mintNftByFront(array, catInfo);
    return catNft;
  }
});
